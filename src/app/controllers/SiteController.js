...
class Paging {
  ...
  calculatePage(req) {
    ...
    if (req.body.btnHome) //click home
      this.index = 0;
    if (req.body.btnEnd) //click end
      this.index = totalPage - 1;
    if (req.body.btnPre) //click pre
      this.index--;
    if (req.body.btnNext) //click next
      this.index++;
    if (req.body.btnIndex) // click button i
  ...
  calculatePageTask(req) {
    ...
    if (req.body.btnHomeTask) //click home
      this.index = 0;
    if (req.body.btnEndTask) //click end
      this.index = totalPage - 1;
    if (req.body.btnPreTask) //click pre
      this.index--;
    if (req.body.btnNextTask) //click next
      this.index++;
    if (req.body.btnIndexTask) // click button i
...
function getSubmissions(req) {
  if (req.session.admin)
    return Submission.find({}).sort({ createAt: 'desc' }).lean().then((submissions) => { return submissions });
  else
    return Submission.find({ user_name: req.session.user }).sort({ createAt: 'desc' }).lean().then((submissions) => { return submissions });
}
