    ...
    submissionsTable: function (submissions, begin, end, isAdmin) {
        ...
        if (isAdmin)
            for (let i = begin; i < end + 1; i++)
                list += "<tr class='submission_item'><th scope='row'>" + (i + 1) + "</th><td>" + submissions[i].createAt + "</td><td>" + submissions[i].user_name + "</td><td> <a href='/homepage/tasks/" + submissions[i].slug + "'>" + submissions[i].task_name + "</a></td><td>" + submissions[i].status + "</td></tr>";
        else
            for (let i = begin; i < end + 1; i++)
                list += "<tr class='submission_item'><th scope='row'>" + (i + 1) + "</th><td>" + submissions[i].createAt + "</td><td> <a href='/homepage/tasks/" + submissions[i].slug + "'>" + submissions[i].task_name + "</a></td><td>" + submissions[i].status + "</td></tr>";
        return list;
    },
